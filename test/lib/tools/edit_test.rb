...
class Tools::EditTest < ActiveSupport::TestCase
  ...
  test "call replaces exact text and returns a diff" do
    path = write_file("a.rb", "def hello\n  puts 'hi'\nend\n")
    result = @tool.call("path" => path, "old_text" => "  puts 'hi'", "new_text" => "  puts 'hello'")
  ...
  test "call deletes text when new_text is empty" do
    path = write_file("b.rb", "line1\nremove_me\nline3\n")
    @tool.call("path" => path, "old_text" => "remove_me\n", "new_text" => "")

    assert_equal "line1\nline3\n", File.read(path)
  end

  test "call returns diff header with file path" do
    path = write_file("c.rb", "foo\nbar\n")
    result = @tool.call("path" => path, "old_text" => "foo", "new_text" => "baz")
  ...
  test "call falls back to fuzzy match when indentation differs" do
  ...
  test "call returns error for blank path" do
    result = @tool.call("path" => "", "old_text" => "x", "new_text" => "y")

    assert_kind_of Hash, result
    assert_match(/blank/i, result[:error])
  end

  test "call returns error for blank old_text" do
    path = write_file("e.rb", "content\n")
    result = @tool.call("path" => path, "old_text" => "", "new_text" => "y")

    assert_kind_of Hash, result
    assert_match(/blank/i, result[:error])
  end

  test "call returns error when file not found" do
    result = @tool.call("path" => "/nonexistent/file.rb", "old_text" => "x", "new_text" => "y")
  ...
  test "call returns error when path is a directory" do
    result = @tool.call("path" => @tmpdir, "old_text" => "x", "new_text" => "y")

    assert_kind_of Hash, result
    assert_match(/directory/i, result[:error])
  end

  test "call returns error when old_text not found in file" do
    path = write_file("f.rb", "hello world\n")
    result = @tool.call("path" => path, "old_text" => "nonexistent text", "new_text" => "y")

    assert_kind_of Hash, result
    assert_match(/could not find/i, result[:error])
  end

  test "call returns error when old_text matches multiple locations" do
    path = write_file("g.rb", "foo\nfoo\n")
    result = @tool.call("path" => path, "old_text" => "foo", "new_text" => "bar")

    assert_kind_of Hash, result
    assert_match(/2 matches/i, result[:error])
  end

  test "call returns error when old_text and new_text are identical" do
    path = write_file("h.rb", "same content\n")
    result = @tool.call("path" => path, "old_text" => "same content", "new_text" => "same content")
  ...
  test "params_schema returns an object schema with path, old_text, and new_text properties" do
    schema = Tools::Edit.new.params_schema
  ...
  test "call returns error when file exceeds max_file_size" do
    ...
    begin
      ...
      result = @tool.call("path" => path, "old_text" => "x", "new_text" => "y")
  ...
  test "call resolves relative path when shell_session provides pwd" do
    dir = @tmpdir
    ...
    tool.call("path" => "relative.rb", "old_text" => "original", "new_text" => "replaced")
