...
class Tools::WriteTest < ActiveSupport::TestCase
  ...
  test "execute creates a new file with given content" do
  ...
  test "execute returns confirmation with byte count and path" do
    path = File.join(@tmpdir, "hello.txt")
  ...
  test "execute overwrites existing file" do
  ...
  test "execute creates intermediate directories automatically" do
  ...
  test "execute returns error when path is a directory" do
    result = @tool.execute({"path" => @tmpdir, "content" => "content"})
    assert_kind_of Hash, result
    assert_match(/directory/i, result[:error])
  end

  test "execute returns permission denied when parent directory is not writable" do
  ...
  test "execute writes empty content" do
    path = File.join(@tmpdir, "empty.txt")
    @tool.execute({"path" => path, "content" => ""})

    assert File.exist?(path)
    assert_equal "", File.read(path)
  end

  test "execute preserves exact content without normalization" do
  ...
  test "input_schema returns an object schema with path and content properties" do
    schema = Tools::Write.input_schema
  ...
  test "execute resolves relative path against working directory when shell_session provided" do
    dir = @tmpdir  # capture as local so the block closes over it, not self
    shell = Object.new
    shell.define_singleton_method(:pwd) { dir }
    tool = Tools::Write.new(shell_session: shell)

    tool.execute({"path" => "relative.txt", "content" => "relative write"})
